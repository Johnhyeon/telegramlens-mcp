...
_BRIEFING_PLAYBOOK = (
    ...
    "   본문: ① 오늘 무슨 일이 있었는지 한 문단(서킷브레이커·사이드카 같은 큰 사건과, 가능하면 "
    "외국인·기관·개인 순매도 등 수급 수치까지). ② '핵심 원인 N'을 번호로 2~4개 — 각각 '왜'까지 "
    "한두 문장으로 설명(macro_거시버즈·웹으로 근거). ③ 간밤 미국·글로벌 한 문단(주요 지수·이슈). "
    "④ 환율·유가·다음 주 일정 같은 '앞으로의 변수' 한두 줄.\n"
    "   오늘 매크로 이벤트(FOMC·PCE·CPI·GDP·고용·관세 등)가 있으면 충분히 짚으세요. '회자되는 중/"
    "거론되는 분위기'로 얼버무리지 말고, 수치·채널·웹 근거를 들어 단정적으로 쓰세요.\n"
...
def _briefing_session(krx: dict) -> str:
    ...
    if status == "after_hours":
        return (
            "정규장 마감 후, 애프터마켓 진행 중(20:00까지). 오늘 정규장을 '왜 이렇게 됐나' "
            "복기하고 이어질 미국장 관전포인트와 내일 전망을 덧붙이세요. 애프터마켓 가격·등락은 "
            "아직 움직이는 값이라 정규장 종가·등락률처럼 쓰지 마세요."
        )
